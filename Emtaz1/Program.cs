...
builder.Services.AddDbContext<EMT_DBContext>(options =>
    options.UseSqlServer(str));
builder.Services.AddDatabaseDeveloperPageExceptionFilter();

builder.Services.AddDefaultIdentity<IdentityUser>(options => options.SignIn.RequireConfirmedAccount = false)
    .AddRoles<IdentityRole>()
    .AddEntityFrameworkStores<EMT_DBContext>();
builder.Services.AddControllersWithViews();

builder.Services.AddScoped<IEmployee,EmployeeDAL>();
builder.Services.AddScoped<IDepartment,DepartmentDAL>();
builder.Services.AddScoped<IEvaluation,EvaluationDAL>();
builder.Services.AddScoped<IJob,JobDAL>();
